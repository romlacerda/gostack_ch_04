...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductsWithNoQtyAvailable = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity,
    );

    if (findProductsWithNoQtyAvailable.length) {
      throw new AppError(
        `The quantity ${findProductsWithNoQtyAvailable[0].quantity} is not available for ${findProductsWithNoQtyAvailable[0].id}`,
      );
    }
